document.addEventListener('DOMContentLoaded', function () {
    ...
    // Gestionnaire d'événements pour les boutons Défaire et Refaire
...
function createFloor() {
    ...
    const floorMaterial = new THREE.MeshStandardMaterial({
        ...
        wireframe: false,
        map: null,
        bumpScale: 0.05, // Ajoute un léger relief pour les joints
    });
...
function applyTileToFloor(texture) {
    ...
    // Applique une légère échelle des UV pour créer des joints clairs
    ...
    console.log(`Carrelage appliqué au sol avec ${repeatX.toFixed(2)}x${repeatY.toFixed(2)} répétitions et joints clairs.`);
}
...
function applyPaintToAllWalls(color) {
    walls.forEach(wall => {
        ...
        saveState(); // Sauvegarde l'état après chaque changement
    });
...
function onMouseClick(event) {
    ...
    if (intersects.length > 0) {
        ...
        if (clickedObject === floor && clickedObject.material.map) {
            ...
            saveState(); // Sauvegarde l'état après chaque rotation
        ...
        if (walls.includes(clickedObject)) {
            ...
            clickedObject.material.needsUpdate = true;
            saveState(); // Sauvegarde l'état après chaque changement de couleur
        }
    }
}

function saveState() {
    // Sauvegarde l'état actuel de la scène
    undoStack.push(scene.toJSON());
    redoStack.length = 0; // Réinitialise la pile redo
}

function undoAction() {
    if (undoStack.length > 0) {
        redoStack.push(scene.toJSON()); // Sauvegarde l'état actuel pour le refaire
...
function redoAction() {
    if (redoStack.length > 0) {
        undoStack.push(scene.toJSON()); // Sauvegarde l'état actuel pour l'annuler
